...
def get_token() -> tuple[str, bool]:
	copilot_token = (os.getenv(COPILOT_TOKEN_ENV) or "").strip()
	if copilot_token and not looks_like_placeholder_secret(copilot_token):
		return copilot_token, True

	pat_token = (os.getenv(PAT_TOKEN_ENV) or "").strip()
	if pat_token and not looks_like_placeholder_secret(pat_token):
		return pat_token, False

	if copilot_token or pat_token:
		raise RuntimeError(
			f"{COPILOT_TOKEN_ENV}/{PAT_TOKEN_ENV} appears to use placeholder values. "
			"Set a real token in .env before running conversion."
		)
...
def main() -> None:
	...
	if attempted_count > 0 and success_count == 0:
		raise RuntimeError(
			"No HTML files were converted successfully. Check token authorization and endpoint settings."
		)
